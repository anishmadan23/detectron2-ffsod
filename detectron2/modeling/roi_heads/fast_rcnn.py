...
def fast_rcnn_inference_with_proposals(
    boxes: List[torch.Tensor],
    scores: List[torch.Tensor],
    image_shapes: List[Tuple[int, int]],
    score_thresh: float,
    ...
    box_feats: torch.Tensor,
    img_embedding_clip: torch.Tensor,
    file_names: str=None,
):
    """
    Call `fast_rcnn_inference_single_image` for all images.
    ...
    """
    # print(len(boxes))
    assert(len(boxes)==1), "Check length of boxes in list"

    result_per_image = [
        fast_rcnn_inference_single_image_with_proposals(
            boxes_per_image, scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image, proposals, box_feats, img_embedding_clip, file_names=file_names
        )
        for scores_per_image, boxes_per_image, image_shape in zip(scores, boxes, image_shapes)
    ]
    return [x[0] for x in result_per_image], [x[1] for x in result_per_image]
...
def fast_rcnn_inference_single_image(
    ...
    scores = scores[:, :-1]     #get rid of background class
...
def fast_rcnn_inference_single_image_with_proposals(
    boxes,
    scores,
    image_shape: Tuple[int, int],
    score_thresh: float,
    ...
    if not valid_mask.all():
        ...
        proposals[0]._fields['proposal_boxes'].tensor = proposals[0]._fields['proposal_boxes'].tensor[valid_mask]
        proposals[0]._fields['objectness_logits'] = proposals[0]._fields['objectness_logits'][valid_mask]
        box_feats = box_feats['valid_mask']
        img_embedding_clip = img_embedding_clip['valid_mask']

    scores = scores[:, :-1]         # get rid of last column? (maybe due to background or something?)
    num_bbox_reg_classes = boxes.shape[1] // 4
    # Convert to Boxes to use the `clip` function ...
    boxes = Boxes(boxes.reshape(-1, 4))
    boxes.clip(image_shape)
    boxes = boxes.tensor.view(-1, num_bbox_reg_classes, 4)  # R x C x 4        (1,1,4)
    ...
    # R' x 2. First column contains indices of the R predictions;
    # Second column contains indices of classes.
    filter_inds = filter_mask.nonzero()                     # this can give same index multiple times (for eg. if class 2 and 8 both are above score_thresh for a sample)
    if num_bbox_reg_classes == 1:
        boxes = boxes[filter_inds[:, 0], 0]
        proposals[0]._fields['proposal_boxes'].tensor = proposals[0]._fields['proposal_boxes'].tensor[filter_inds[:, 0]]
        proposals[0]._fields['objectness_logits'] = proposals[0]._fields['objectness_logits'][filter_inds[:, 0]]
        box_feats = box_feats[filter_inds[:,0]] 
        img_embedding_clip = img_embedding_clip[filter_inds[:,0]] 
    else:
        boxes = boxes[filter_mask]
    scores = scores[filter_mask]
    ...
    keep = batched_nms(boxes, scores, filter_inds[:, 1], nms_thresh)    # should always be 1 in the config as we do nms later. Calling nms with 1, sorts predictions according to scores.
    if topk_per_image >= 0:
        keep = keep[:topk_per_image]
    boxes, scores, filter_inds = boxes[keep], scores[keep], filter_inds[keep]
    box_feats = box_feats[keep]
    img_embedding_clip = img_embedding_clip[keep]
    proposals[0]._fields['proposal_boxes'].tensor = proposals[0]._fields['proposal_boxes'].tensor[keep]
    proposals[0]._fields['objectness_logits'] = proposals[0]._fields['objectness_logits'][keep]

    result = Instances(image_shape)
    result.pred_boxes = Boxes(boxes)
    result.scores = scores
    result.pred_classes = filter_inds[:, 1]
    result._file_name = file_names[0]     # need to use _ as Instances only takes values which have same number of instances in the first dimension.
    
    result.objectness_logits = proposals[0]._fields['objectness_logits']
    result.proposal_boxes = Boxes(proposals[0]._fields['proposal_boxes'].tensor)
    result.box_feats = box_feats
    result.img_embedding_clip = img_embedding_clip
    return result, filter_inds[:, 0]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def from_config(cls, cfg, input_shape):
        return {
            ...
            # "get_fed_loss_cls_weights"  : lambda: torch.zeros([18]),
